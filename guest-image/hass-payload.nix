...
#
# Imported alongside configuration.nix (see ./hass-guest.nix) by the HA guest
...
{ pkgs, ... }:
{
  briard.payload = {
    ...
    # HTTP stack is up — the same readiness signal hass-payload.nix waits on, rather than `/`,
